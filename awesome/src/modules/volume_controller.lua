...
return function(s)
	...
	local function get_source_devices()
		awful.spawn.easy_async_with_shell(
			[[ pactl list sinks | grep -E 'node.name|device.description' | awk '{gsub(/"/, ""); for(i = 3;i < NF;i++) printf $i " "; print $NF}' ]],
...
bluez_output.14_3F_A6_1C_7B_D4.a2dp-sink
			--]]
		...
		awful.spawn.easy_async_with_shell("./.config/awesome/src/scripts/mic.sh volume", function(stdout)
			...
			if volume > 0 then
				volume_controller
					...
					:set_image(gears.color.recolor_image(icondir .. "microphone.svg", color["LightBlue200"]))
			else
				volume_controller
					:get_children_by_id("mic_volume_margin")[1].icon
					:set_image(gears.color.recolor_image(icondir .. "microphone-off.svg", color["LightBlue200"]))
			end
		end)
	end
	...
	awesome.connect_signal("get::volume", function(volume)
		...
		volume_controller.controller_margin.controller_layout.audio_volume_margin.audio_volume.icon:set_image(
			gears.color.recolor_image(icon .. ".svg", color["Purple200"])
		)
	end)
	...
	awesome.connect_signal("get::volume_mute", function(mute)
		if mute then
			volume_controller.controller_margin.controller_layout.audio_volume_margin.audio_volume.icon:set_image(
				gears.color.recolor_image(icondir .. "volume-mute.svg", color["Purple200"])
			)
		end
	end)
