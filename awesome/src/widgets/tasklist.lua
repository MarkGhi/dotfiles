...
local list_update = function(widget, buttons, label, data, objects)
	...
	for _, object in ipairs(objects) do
		...
		if object == client.focus then
			...
			task_widget:set_bg(color.light4)
			task_widget:set_fg(color.dark0)
			task_widget.container.layout_it.title:set_text(text)
		else
			task_widget:set_bg(color.dark1)
		...
		task_widget:connect_signal("mouse::enter", function()
			...
			if object == client.focus then
				task_widget.bg = color.light4 .. "dd"
			else
				task_widget.bg = color.dark2 .. "dd"
			end
