...
local list_update = function(widget, buttons, label, data, objects)
	...
	for _, object in ipairs(objects) do
		...
		elseif object == awful.screen.focused().selected_tag then
			tag_widget:set_bg(color.light4)
			tag_widget:set_fg(color.dark0)
		else
			tag_widget:set_bg(color.dark1)
			tag_widget:set_fg(color.light2)
		end
		...
		tag_widget:connect_signal("mouse::enter", function()
			...
			if object == awful.screen.focused().selected_tag then
				tag_widget.bg = color.light4 .. "dd"
			else
				tag_widget.bg = color.dark2 .. "dd"
			end
